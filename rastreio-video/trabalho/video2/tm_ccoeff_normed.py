...
for indice, nome_quadro in enumerate(sorted(os.listdir('video2/frames_cinza'))):
    ...
    # no método TM_CCOEFF_NORMED, valores maiores indicam melhores correspondências
    if max_val > 0.8:  # ajuste este limiar conforme necessário
        top_left = max_loc
        bottom_right = (top_left[0] + w, top_left[1] + h)
        cv2.rectangle(frame, top_left, bottom_right, 255, 2)
        cv2.imwrite(os.path.join('video2/frames_rastreio_ccoeff_normed', nome_quadro), frame)
