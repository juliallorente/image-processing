...
template_path = 'video1/template.jpg'
...
for indice, nome_quadro in enumerate(sorted(os.listdir('video1/frames_cinza'))):
    ...
    # no método TM_SQDIFF_NORMED, valores menores indicam melhores correspondências
    if min_val < 0.2:  # ajuste este limiar conforme necessário
        top_left = min_loc
        bottom_right = (top_left[0] + w, top_left[1] + h)
        cv2.rectangle(frame, top_left, bottom_right, 255, 2)
        cv2.imwrite(os.path.join('video1/frames_rastreio_sqidiff_normed', nome_quadro), frame)
